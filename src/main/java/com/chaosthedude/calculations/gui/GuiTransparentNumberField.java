...
public class GuiTransparentNumberField extends Gui {
	...
	public GuiTransparentNumberField(int id, FontRenderer fontRenderer, int x, int y, int width, int height) {
		this.id = id;
	...
	public boolean textboxKeyTyped(char typedChar, int keyCode) {
		if (!isFocused) {
			...
			if (isEnabled) {
				writeText(filterText(GuiScreen.getClipboardString()));
			}
			...
			case Keyboard.KEY_DELETE:
				...
				return true;
			default:
				if ((typedChar == '-' && cursorPosition == 0 && !text.contains("-")) || (typedChar >= 48 && typedChar <= 57)) {
					if (isEnabled) {
